...
var StdInReadStream = require('../../util/stdinreadstream');
  ...
  function uploadAzureBlob(file, container, blobName, options, _) {
    ...
    try {
      if (specifiedFileName === "-") {
         var stdinStream = new StdInReadStream();
         ...
         performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, stdinStream, 0 /* streamLength */, storageOptions);
      } else {
        if (blobTypeName.toLowerCase() === 'page') {
          //Upload page blob
