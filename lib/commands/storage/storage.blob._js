...
var FileReadStream = require('./node_modules/azure-storage/lib/commom/common/streams/filereadstream');
...
exports.init = function(cli) {
  ...
  */
  ...
  blob.command('show [container] [blob]')
    ...
    .option('--lease <leaseId>', $('the lease ID'))
    .option('--snapshot <snapshotId>', $('the snapshot ID'))
    .option('--sas <sas>', $('the shared access signature'))
    .addStorageAccountOption()
  ...
  copy.command('start [sourceUri] [destContainer]')
    ...
    .option('--snapshot <snapshot>', $('the snapshot id if source is a snapshot'))
  ...
  function uploadAzureBlob(file, container, blobName, options, _) {
    ...
    try {
      if (specifiedFileName === "-") {
         var fileStreamFromStdin = new FileReadStream('/dev/stdin');
         fileStreamFromStdin._fd = 0; // Pull data form stdin
         if (blobTypeName.toLowerCase() === 'page') {
           //Upload page blob
           operation = getStorageBlobOperation(blobService, 'createPageBlobFromStream');
         } else if (blobTypeName.toLowerCase() === 'block'){
           //Upload block blob
           operation = getStorageBlobOperation(blobService, 'createBlockBlobFromStream');
         } else if (blobTypeName.toLowerCase() === 'append') {
           //Upload append blob
           storageOptions.absorbConditionalErrorsOnRetry = true;
           operation = getStorageBlobOperation(blobService, 'createAppendBlobFromStream');
         }
         performStorageOperation(operation, _, specifiedContainerName, specifiedBlobName, fileStreamFromStdin, 0 /* streamLength */, storageOptions);
      } else {
        if (blobTypeName.toLowerCase() === 'page') {
          //Upload page blob
